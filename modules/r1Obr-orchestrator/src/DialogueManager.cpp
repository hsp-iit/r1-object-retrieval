...
void DialogueManager::interactWithDialogMng(const std::string& msgIn)
{
    ...
    switch(cmdType){
        ...
        case dlgmsg::CmdTypes::GRASP: {
            // Grasping - send command to /new-task/rpc:i
            yCWarning(DIALOG_MNG_ORCHESTRATOR) << "GRASP is the way" << msgIn;
            yarp::os::Bottle command, response;
            command.addString("start_video");
            command.addString("cris_grasp_position");
            yarp::os::RpcClient gbNewTaskPort;
            if (!gbNewTaskPort.open("/dialogueManager/gbNewTask:rpc")) {
                yCError(DIALOG_MNG_ORCHESTRATOR, "Unable to open RPC client port for /new_task/rpc:i");
                return;
            }
            if (!yarp::os::Network::connect(gbNewTaskPort.getName(), "/new_task/rpc:i")) {
                yCError(DIALOG_MNG_ORCHESTRATOR, "Unable to connect to /new_task/rpc:i");
                gbNewTaskPort.close();
                return;
            }
            gbNewTaskPort.write(command, response);
            yCInfo(DIALOG_MNG_ORCHESTRATOR, "Response from /new_task/rpc:i: %s", response.toString().c_str());
            gbNewTaskPort.close();
